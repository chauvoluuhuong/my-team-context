...
/** @returns {Promise<{stored: "keychain" | "file", warning?: string}>} */
...
/** @returns {Promise<{repo?: string, defaultBranch?: string, selectedAt?: string, recent?: string[]}>} */
...
export async function setActiveRepo(repo, defaultBranch) {
  const previous = await readState();
  const recent = [repo, ...(previous.recent ?? []).filter((r) => r !== repo)].slice(0, 8);
  const state = { repo, defaultBranch, selectedAt: new Date().toISOString(), recent };
  await writeState(state);
  return state;
}

export async function clearActiveRepo() {
  const previous = await readState();
