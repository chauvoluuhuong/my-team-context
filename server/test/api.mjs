...
const routes = {
  ...
  "/user/repos": [
    { full_name: "octo/app", private: true, default_branch: "main", description: "The app", language: "TypeScript", pushed_at: "2026-08-01T00:00:00Z" },
    { full_name: "octo/site", private: false, default_branch: "main", description: null, language: "CSS", pushed_at: "2026-07-01T00:00:00Z" },
  ],
};
...
process.env.REPO_CONTEXT_API = `http://127.0.0.1:${server.address().port}`;

// Imported after the env vars are set — the modules read them at load time.
const { listRepos, listFiles, readFile, searchCode, overview, repoMeta } = await import("../src/github.js");
const { setActiveRepo, readState } = await import("../src/store.js");
...
assert.deepEqual(dirListing.entries.map((e) => e.path), ["src", "README.md"], "dirs sort first");
...
assert.deepEqual(tree.entries.map((e) => e.path), ["src/index.ts", "src/lib/util.ts"], "prefix filter");
...
await assert.rejects(() => readFile({ path: "nope.ts" }), /GitHub error: 404/, "404s surface readably");
await assert.rejects(() => listFiles({ repo: "not-a-repo" }), /owner\/name/, "repo form is validated");
